...
export default function BaselineDrift() {
  ...
  const lowDriftStart = useRef<number | null>(null);
  const baselineTriggerFired = useRef(false);
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      setDrift((prev) => {
        const val = prev * 0.7 + next * 0.3;
        window.dispatchEvent(new CustomEvent('baseline:drift', { detail: { value: val } }));
        return val;
      });
    }, 200);
  ...
  useEffect(() => {
    if (baselineTriggerFired.current) return;
    if (drift < 15) {
      if (lowDriftStart.current === null) {
        lowDriftStart.current = Date.now();
      } else if (Date.now() - lowDriftStart.current > 20000) {
        window.dispatchEvent(new CustomEvent('narrator:trigger:baseline'));
        baselineTriggerFired.current = true;
      }
    } else {
      lowDriftStart.current = null;
    }
  }, [drift]);
