...
const MOOD_COLORS: Record<string, string> = {
  tense: '#ff006e', gentle: '#e040fb', cold: '#00d4ff',
  neutral: '#6b6660', dark: '#ff006e', warm: '#e040fb',
};
            ...
            {TRAIT_KEYS.map((k) => (
              <div key={k} className="flex flex-col items-center gap-0.5">
                <div className="w-3 h-10 bg-[#1a1712] relative overflow-hidden rounded-sm">
                  <motion.div animate={{ height: `${(traits[k] / maxTrait) * 100}%` }} className="absolute bottom-0 w-full bg-[#e040fb]/70 rounded-sm" />
                </div>
          ...
          <div className="flex justify-center gap-6 mt-8">
            {TRAIT_KEYS.map((k) => (
              <div key={k} className="flex flex-col items-center gap-2">
                <motion.div initial={{ height: 0 }} animate={{ height: `${Math.min(traits[k] * 8, 80)}px` }} transition={{ duration: 2, delay: 0.3 }} className="w-5 bg-[#e040fb]/60 rounded-sm" />
                <span className="text-[10px] text-[#6b6660] font-mono">{k.slice(0, 3)}</span>
                <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5 }} className="text-xs text-[#e040fb]">
                  {traits[k].toFixed(1)}
                </motion.span>
              </div>
            ))}
        ...
        <div className="max-w-xl w-full px-6 space-y-8">
            ...
            {TRAIT_KEYS.map((k) => (
              <div key={k} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-2 bg-[#1a1712] rounded-sm overflow-hidden">
                  <motion.div initial={{ width: 0 }} animate={{ width: `${(traits[k] / maxVal) * 100}%` }} transition={{ duration: 1 }} className="h-full bg-[#e040fb] rounded-sm" />
                </div>
                <span className="text-xs text-[#e040fb] w-8 text-right">{traits[k].toFixed(0)}</span>
              </div>
            ))}
          </div>
          ...
          {corroboration && (
            <motion.div
              ...
              className="border border-[#00d4ff]/30 p-4 bg-[#091418]/40"
            >
              <div className="text-[10px] font-mono text-[#00d4ff] tracking-widest mb-3">
                CROSS-REFERENCE · FIELD LOG · {dominantTrait.toUpperCase()}
              </div>
              <p className="text-sm md:text-base text-[#dfd9cd] leading-relaxed italic border-l border-[#00d4ff]/40 pl-4">
          ...
          <div className="pt-4 border-t border-[#1f1c17] flex items-center justify-between">
            ...
            <button onClick={() => setState('idle')} className="text-xs font-mono text-[#00d4ff] hover:text-[#e040fb] transition-colors">
              DISMISS
            </button>
          </div>
        </div>
